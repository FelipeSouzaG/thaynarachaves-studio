...
export async function showModalTraining() {
  ...
  const imageSources = [
    './src/img/curso-banner.svg',
    './src/img/curso_lash_2.svg',
    './src/img/curso_lash_3.svg',
    './src/img/curso_lash_4.svg',
    './src/img/curso_lash_5.svg',
    './src/img/curso_lash_6.svg',
  ];
...
async function showModalInfoCarousel(
  titleCarousel,
  imageCarousel,
  textCarousel
) {
  ...
  content.innerHTML = `
      <div class="carousel-modal">
        <img src="${imageCarousel}" alt="Conteúdo de treinamento de Lash desing">
        <label class="label">${textCarousel}</label>
      </div>
    `;
...
export async function setupCarouselModalListeners() {
  ...
  slides.forEach((slide) => {
    slide.addEventListener('click', async () => {
      ...
      // Usa os dados capturados para abrir o modal
      await showModalInfoCarousel(title, imageSrc, text);
    });
  });
}
